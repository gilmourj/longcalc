...
# DE positives
def de(f):
	print("Run at: ", datetime.now(), file=f)
...
def me(f):
	print("Run at: ", datetime.now(), file=f)
	print(pd.read_csv("https://gateway.maine.gov/dhhs-apps/mecdc_covid/hospital_capacity.csv", nrows=1, usecols=[0,1,2,3]), file=f)
	print("\n\n", file=f)
...
# WV Testing
def wv(f): 
    print("Last run at: ", datetime.now().isoformat(), file=f)
